...
export default function Peice({ ChessPiece, rank, file, handleClick, activeTile, inActiveTile }) {
    ...
    return (
        <button className={`absolute h-[12.5%] aspect-square flex items-center justify-center text-[calc(min(100vw,100vh)/14)] p-${rank}${file} ${ChessPiece <= 5 ? "text-white *drop-shadow-[0_0_1px_black]" : "*:drop-shadow-[0_0_1px_white]"} ${isActiveTile && "bg-green-700/50"} ${isInActiveTile && "bg-blue-700/35"}`} draggable={true} onDragStart={handleDragStart} onDragEnd={handleDragEnd} onClick={e => handleClick({ rank, file, ChessPiece })} >
            {CPS[ChessPiece]}
        </button>
    )
}
