...
export default function AlertMessage({ message1, message2, light, handleNewGame, cancelAlert }) {
  return (
    <div className='h-screen w-full p-4 fixed top-0 left-0 bg-black/50 flex items-center justify-center'>
      <div className={`w-full max-w-[400px] h-full max-h-[350px] bg-gray-700 bg-[url('/image/gameEnd.jpg')] bg-cover bg-no-repeat bg-center rounded shadow-[0px_3px_10px_white]`}>
        <div className={`w-full max-w-[400px] h-full max-h-[350px] py-5 p-8 rounded flex gap-1 flex-col items-center justify-center drop-shadow-[0px_3px_10px_white] bg-black/30 `}>
          ...
          <button className="py-1 px-4 text-white rounded font-extrabold bg-cyan-700 hover:bg-cyan-600 hover:scale-105 shadow-[1px_2px_3px_white] flex gap-1 items-center " onClick={handleNewGame}><FaArrowRotateRight   /> play again</button>
          <button className="py-1 px-4 text-white rounded font-extrabold bg-cyan-700 hover:bg-cyan-600 hover:scale-105 shadow-[1px_2px_3px_white] flex gap-1 items-center" onClick={cancelAlert}><FaXmark /> Cancel</button>
        </div>
      </div>
    </div>
  )
